...
namespace mapKnight.Android.CGL
{
	public class CGLCamera
	{
		...
		public void Update ()
		{
			...
			CurrentMapTile.X = Math.Min (Math.Max ((int)CharacterTile.X - (int)(Content.Map.DrawSize.Width / 2), 0), (int)(Content.Map.Width - Content.Map.DrawSize.Width));
			if (CharacterTile.X > Content.Map.DrawSize.Width / 2f && CharacterTile.X < (Content.Map.Width - Content.Map.RealDrawSize.Width / 2f)) {
				...
				// on the right side
				charOffsetX = -Content.ScreenRatio + 2f * ((Content.Map.Width - CharacterTile.X) / Content.Map.DrawSize.Width) * Content.ScreenRatio;
			} else {
				// on the left side
				charOffsetX = 2f * ((Content.Map.DrawSize.Width / 2 - CharacterTile.X) / Content.Map.RealDrawSize.Width) * Content.ScreenRatio;
			}

			if (CharacterTile.Y > (1 - characterOffset) * Content.Map.RealDrawSize.Height / 2 && CharacterTile.Y < (Content.Map.Height - (1 + characterOffset) * Content.Map.DrawSize.Height / 2)) {
				...
				charOffsetY = 1 - 2f * (Content.Map.Height - (CharacterCenter.Y / PhysX.PhysXMap.TILE_BOX_SIZE)) / ((float)Content.Map.DrawSize.Height);
