...
namespace mapKnight.Android.CGL {
    public class CGLSprite2D : CGLTexture2D {
        private Dictionary<string, float[]> sprites;

        public CGLSprite2D (Dictionary<string, int[]> initialSprites, int texture, string name, int width, int height) : base (texture, name, width, height) {
            sprites = new Dictionary<string, float[]> ();
            // parse sprite pixel coords to opengl coords
            foreach (KeyValuePair<string, int[]> sprite in initialSprites) {
