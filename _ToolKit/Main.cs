...
namespace mapKnight.ToolKit {
    public partial class Main : Form {
        ...
        private void Main_Load (object sender, EventArgs e) {
            tbctrl_main.Controls[0].BackColor = Color.Black;
            ...
            imagesizebar.TrackBar.TrackBar.ValueChanged += (object tsender, EventArgs te) => {
                ...
                tiles.LargeImageList.ImageSize = new Size (iListViewImageSize, iListViewImageSize);
            };

            initCharacterEdit ();
            initAnim ();

            tiles.SelectedIndices.Add (0);
                    ...
                    for (int x = 0; x < iTileRenderWidth; x++) {
                        for (int y = 0; y < iTileRenderHeight; y++) {
                            if (x + hscrlbar_map.Value < iMaps[iSelectedMap].Width && y + vscrlbar_map.Value < iMaps[iSelectedMap].Height) {
                                ...
                                g.Graphics.DrawImage (tileImages[iMaps[iSelectedMap].Data[x + hscrlbar_map.Value, y + vscrlbar_map.Value, 0]], x * iTileSize + spltcntr_map.Right, y * iTileSize + tlstrp_map.Bottom, iTileSize, iTileSize);
                                g.Graphics.DrawImage (tileImages[iMaps[iSelectedMap].Data[x + hscrlbar_map.Value, y + vscrlbar_map.Value, 1]], x * iTileSize + spltcntr_map.Right, y * iTileSize + tlstrp_map.Bottom, iTileSize, iTileSize);
                                g.Graphics.DrawImage (tileImages[iMaps[iSelectedMap].Data[x + hscrlbar_map.Value, y + vscrlbar_map.Value, 2]], x * iTileSize + spltcntr_map.Right, y * iTileSize + tlstrp_map.Bottom, iTileSize, iTileSize);
                            }
                        }
                    }
        ...
        private void updateTextures () {
            tiles.LargeImageList.Images.Clear ();
            tiles.LargeImageList.Images.AddRange (tileSets[selectedTileSet].GetBitmapsOrdered ());
            tileImages = tiles.LargeImageList.Images.Cast<Bitmap> ().ToArray ();
        }
        ...
        private void tbpg_mapeditor_MouseDown (object sender, MouseEventArgs e) {
            if (e.X > spltcntr_map.Right && e.X < vscrlbar_map.Left && e.Y > tlstrp_map.Bottom && e.Y < hscrlbar_map.Top && iSelectedMap > -1) {
                ...
                if (e.Button == MouseButtons.Left && clickedTile.X < iMaps[iSelectedMap].Width && clickedTile.Y < iMaps[iSelectedMap].Height) {
                    ...
                    case Tool.Brush:
                        iMaps[iSelectedMap].Data[clickedTile.X, clickedTile.Y, selectedLayer] = selectedTile[selectedLayer];
                        iClicked = true;
                        break;
                    case Tool.Eraser:
                        ...
                        break;
                    case Tool.Bucket:
                        iMaps[iSelectedMap].Fill (clickedTile.X, clickedTile.Y, new int[] { });
                        break;
                    ...
                    case Tool.Select:
                        selectedLayer = 0;
                        tiles.Items[(int)iMaps[iSelectedMap].Data[clickedTile.X, clickedTile.Y, 0]].Selected = true;
                        tiles.Items[(int)iMaps[iSelectedMap].Data[clickedTile.X, clickedTile.Y, 0]].Focused = true;
                        selectedLayer = 1;
                        tiles.Items[(int)iMaps[iSelectedMap].Data[clickedTile.X, clickedTile.Y, 1]].Selected = true;
                        tiles.Items[(int)iMaps[iSelectedMap].Data[clickedTile.X, clickedTile.Y, 1]].Focused = true;
                        selectedLayer = 2;
                        tiles.Items[(int)iMaps[iSelectedMap].Data[clickedTile.X, clickedTile.Y, 2]].Selected = true;
                        tiles.Items[(int)iMaps[iSelectedMap].Data[clickedTile.X, clickedTile.Y, 2]].Focused = true;
            ...
            if (e.X > spltcntr_map.Right && e.X < vscrlbar_map.Left && e.Y > tlstrp_map.Bottom && e.Y < hscrlbar_map.Top && iClicked) {
                ...
                if (e.Button == MouseButtons.Left && clickedTile.X < iMaps[iSelectedMap].Width && clickedTile.Y < iMaps[iSelectedMap].Height) {
                    ...
                    case Tool.Eraser:
                        iMaps[iSelectedMap].Data[clickedTile.X, clickedTile.Y, 0] = 0;
                        iMaps[iSelectedMap].Data[clickedTile.X, clickedTile.Y, 1] = 0;
                        iMaps[iSelectedMap].Data[clickedTile.X, clickedTile.Y, 2] = 0;
                        break;
                    }
                } else if (e.Button == MouseButtons.Right) {
                    iMaps[iSelectedMap].Data[clickedTile.X, clickedTile.Y, 0] = 0;
        ...
        private void tsb_map_brush_1_Click (object sender, EventArgs e) {
            selectedLayer = 0;
            iSelectedTool = Tool.Brush;
        }

        private void tsb_map_brush_2_Click (object sender, EventArgs e) {
            selectedLayer = 1;
            iSelectedTool = Tool.Brush;
        }

        private void tsb_map_brush_3_Click (object sender, EventArgs e) {
            selectedLayer = 2;
            iSelectedTool = Tool.Brush;
        }
        ...
        private void tiles_SelectedIndexChanged (object sender, EventArgs e) {
            if (tiles.SelectedIndices.Count > 0) {
                tiles.Items[selectedTile[selectedLayer]].BackColor = tiles.BackColor;
                selectedTile[selectedLayer] = tiles.SelectedIndices[0];
                switch (selectedLayer) {
                case 0:
                    tiles.Items[selectedTile[selectedLayer]].BackColor = Color.DarkBlue;
                    break;
                case 1:
                    tiles.Items[selectedTile[selectedLayer]].BackColor = Color.Blue;
                    break;
                case 2:
                    tiles.Items[selectedTile[selectedLayer]].BackColor = Color.LightBlue;
                    break;
                }
                tiles.SelectedIndices.Clear ();
            }
        }
        ...
        private void tsmmi_exportraw_Click (object sender, EventArgs e) {
            if (folderBrowserDialog.ShowDialog () == DialogResult.OK) {
                ...
                foreach (Map map in iMaps) {
                    map.Save (Path.Combine (mappath, map.Name), "");
                }
